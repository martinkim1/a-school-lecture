...
def generate_questions_and_answers(semester):
    if semester == 1:
        ...
        questions = [
				"데이터 (이상현상)은 불필요한 데이터 중복으로 인해 릴레이션 처리 시 발생하는 부작용을 말한다.",
				"데이터 이상 현상 방지를 위해 규칙에 따라 릴레이션을 분할하는 과정을 (정규화)라고 한다.",
				"함수 (종속성）은 같은 릴레이션 안의 속성 간에 특정 속성 값이 함수적으로 다른 속성 값을 결정하는 종속 관계를 말한다.",
				"제1정규형의 문제점을 해결하려면 (부분) 함수 종속성을 제거해야 한다.",
				"(이행성) 함수 종속성은 기본키에 속하지 않은 일반 속성 값이 기본키에 속하지 않는 또 다른 일반 속성 값을 결정함을 의미한다.",
				"E-R 모델은 현실 세계를 (개체)와 관계를 이용하여 개념적 구조로 표현하는 방법이다.",
				"(유도) 속성은 값을 저장하지 않아도 다른 속성 값으로부터 계산될 수 있는 속성으로 점선의 타원으로 표시된다.",
				"보통 강개체와 약개체는 일대다의 (식별) 관계를 맺게 되며 약개체는 관계에 (전체) 참여하게 된다.",
				"E-R 다이어그램에서 (일반화) 관계는 IS-A 관계라고도 하며 역삼각형으로 표현된다.",
				"관계 (카디널리티)는 관계를 맺는 두 개체 집합간의 사상 형태를 정의한다.",
				"데이터베이스 설계의 핵심은 작성된 E-R 다이아그램을 (관계형) 데이터 모델로 변환하는 논리적 데이터 설계 과정이다.",
				"요구사항 명세서에서 고유한 명칭(이름, 번호)을 보유하는 존재는 (개체)로 표현하는 것이 바람직하다.",
				"E-R 다이어그램의 관계는 관계 (카디널리티)에 따라서 관계형 데이터 모델로의 변환 과정이 다르다.",
				"(물리적) 설계는 릴레이션 스키마들을 실제 DBMS 안에 물리적인 저장 구조를 생성하는 SQL 명령어 문장으로 변환한다.",
				"E-R 다이아그램에서 일대다 관계는 하나의 (외래)키 속성으로 변환된다.",
				"ERwin에서 결국 대다다 관계선은 새로운 (중간) 엔터티와의 2개의 일대다 관계선으로 변환되어야만 한다.",
				"ERwin에서 식별 관계는 자식 엔터티를 약 엔터티 즉, (둥근) 사각형으로 표시하고 비식별 관계는 자식 엔터티를 강 엔터티 즉, (직)사각형으로 표시한다.",
				"(데이터웨어하우스)는 조직의 일부 사용자만 사용할 목적으로 주제 범위를 제한하여 구축한 분석 데이터 저장소이다.",
				"(다차원) 모델링은 높은 연관성을 갖는 데이터들을 데이터웨어하우스 안에서 다차원성을 유지하도록 구조화하는 것이다.",
				"다차원 데이터베이스에서 데이터는 '(큐브)' 라고 불리는 특별한 다차원 배열 구조의 '셀'안에 저장된다.",
				"같은 차원 안의 수준들 간의 수직적 상하 관계를 (개념계층)이라 하며 상위 수준의 개념으로부터 하위 수준의 개념으로 차원 멤버들을 계층적으로 조직화한다.",
				"스타 스키마는 하나의 (사실) 테이블을 중심으로 주위에 여러 개의 차원 테이블들이 연결되는 방사성 구조이다.",
				"수집한 데이터들의 상관관계를 분석하면 정보를 발견할 수 있다. 정보를 조직화하면 지식이 만들어지고 지식을 구조화하면 (지혜)가 된다.",
				"데이터웨어하우스, 데이터마트나 데이터마이닝에서는 수집된 데이터를 분석이 용이하도록 통일된 형태로 변환하는 과정을 (ETL)이라고 한다.",
				"하둡 (에코) 시스템은 하둡 프레임워크를 이루고 있는 다양한 서브 프로젝트들의 모임이다.",
				"저가의 서버라도 서버 자체를 더 추가한 뒤 서버드를 묶어 분산 병령 처리함으로싸 전체 시스템의 용량을 키우는 방식을 수평적 또는 스케일 (아웃) 확장 방식이라고 한다.",
				"(데이터 마이닝)은 데이터 집합으로부터 의미있는 통계적 규칙이나 패턴을 찾아냄으로써 데이터의 새로운 가치를 찾아내는 기법이다.",
				"(인포그래픽)은 정보와 그래픽의 합성어로 분석 결과를 쉽게 이해할 수 있도록 그래픽과 텍스트를 조합하여 시각적으로 표현하는 방법이다.",
				"몽고DB에서 가용성을 향상시키기 위한 (샤딩)은 데이터베이스의 데이터를 수평 분할하여 여러 서버 분산 저장하는 방식이다.",
				"몽고DB의 기본 구성 요소중에서 (컬렉션)은 문서의 모음으로 동적 스키마를 갖는 테이블과 같다.",
				"몽고DB (쉘)은 문서에 대한 질의뿐만 아니라 관리나 실행중인 인스턴스를 점검하는데 유용한 도구로써 자바스크립트 해석 기능을 가지고 있어 자바스크립트 프로그램 작성과 실행이 가능하다.",
				"( replaceOne( ) ) 명령어 함수는 특정 문서를 찾아서 새로운 문서로 교체한다.",
				"몽고DB에서 컬렉션에 문서 하나를 추가하는 명령어는 ( insertOne( ) )이다."
        ]
        answers = [
